...
for (const product of Object.values(FREE_TAROT_PACKAGES)) {
  test(`${product.tier} Free Tarot orders recover from v50-v55 reconciliation through the delivery-job boundary`, () => {
...
test('v55 is current while v50-v54 remain explicit backward-recovery aliases', () => {
  assert.equal(FREE_TAROT_FUNNEL_VERSION, 'premium-choice-2026-08-v55');
...
test('reconciliation fails closed for an unknown release, wrong page, or unknown SKU', () => {
  ...
  assert.equal(freeTarotReconciledVariantId({ ...valid, funnelVersion: 'premium-choice-2026-08-v56' }), '');
...
test('v55 paid-new-spread missed-webhook recovery keeps the canonical Free Tarot page and package authority', () => {
  const variantId = freeTarotReconciledVariantId({
    funnelVersion: 'premium-choice-2026-08-v55',
    page: FREE_TAROT_PAGE,
