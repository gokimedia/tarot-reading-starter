...
// ---- page identity pins (canonical constants; the storefront cannot drift) -
import {
  ...
  SEVEN_CARD_HORSESHOE_TYPE,
} from './seven-card-horseshoe-compact.mjs';
const PAGE = SEVEN_CARD_HORSESHOE_PAGE;
const TYPE = SEVEN_CARD_HORSESHOE_TYPE;
const SPREAD = SEVEN_CARD_HORSESHOE_SPREAD;
const PRESENTATION_VARIANT = SEVEN_CARD_HORSESHOE_PRESENTATION_VARIANT;
const SNAPSHOT_VERSION = 'reading-snapshot-v2';
const SHARED_FUNNEL_VERSION = 'enterprise-shared-tools-2026-08-v1';
const SCOPE = SEVEN_CARD_HORSESHOE_SCOPE;
const CONFIDENCE = SEVEN_CARD_HORSESHOE_CONFIDENCE;
...
// ---------------------------------------------------------------------------
// Kısa, kart-temelli, KOŞULLU cevap: yönü söyler, garantiye ve tarihe girmez.
export function deterministicAnswer(cards) {
  ...
  const lean = outcome.reversed
    ? 'leans cautious: the current path needs a change before it settles'
    : 'leans quietly favourable if the current pattern holds';
  return `${outcome.name}${outcome.reversed ? ' reversed' : ''} in the Likely Outcome ${lean}, and ${advice.name} in the Advice slot names the move that tips it.`;
}

const ANSWER_LEAN = /\b(?:yes|no|lean(?:s|ing)?|points?|suggests?|favou?rs?|likely|unlikely|depends|conditional|cautious|not yet|open)\b/i;

export function auditAnswer(answer, cards) {
  const text = clean(answer, 400);
  const words = wordCount(text);
  if (words < 14 || words > 48) return { ok: false, reason: `answer word count ${words} outside 14-48` };
  if (/[–—]/.test(text)) return { ok: false, reason: 'em or en dash in answer' };
  if (FORBIDDEN_FREE.test(text)) return { ok: false, reason: 'answer makes a guarantee or private-state claim' };
  if (DATE_CLAIM.test(text)) return { ok: false, reason: 'answer promises timing' };
  if (!ANSWER_LEAN.test(text)) return { ok: false, reason: 'answer does not state a bounded lean' };
  const suppliedLower = new Set(cards.map((card) => card.name.toLowerCase()));
  let named = 0;
  for (const name of TAROT_CARD_NAMES) {
    const hit = new RegExp(`\\b${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(text);
    if (!hit) continue;
    if (!suppliedLower.has(name.toLowerCase())) return { ok: false, reason: `answer names ${name}, which is not in this spread` };
    named += 1;
  }
  if (named < 1) return { ok: false, reason: 'answer names none of the supplied cards' };
  return { ok: true };
}
...
function insightsPrompt(question, cards) {
  ...
  const system = [
    'You write the free result panel for a seven-card tarot horseshoe on Deckaura.',
    'Grounded, plain-spoken, non-mystical. British-neutral English. No em or en dashes.',
    'Rules:',
    '- Return STRICT JSON: an object {"answer": string, "insights": [3 objects]}.',
    '- "answer": 16 to 42 words. A short, direct, CONDITIONAL answer to the exact question: state which way the spread leans (yes / no / not yet / depends on one condition) and name the one supplied card that carries that lean. Never a guarantee, never a date, never another person\'s private feelings.',
    `- "insights": exactly 3 objects {"label","lead","body"} with labels exactly "${INSIGHT_LABELS[0]}", "${INSIGHT_LABELS[1]}", "${INSIGHT_LABELS[2]}" in that order.`,
...
export async function generateInsights(payload, env, options = {}) {
  ...
  const fallbackAnswer = deterministicAnswer(cards);
  ...
  if (options.deterministicOnly === true || !modelAvailable) {
    return { answer: fallbackAnswer, insights: fallback, source: 'deterministic' };
  }
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      const completion = await completeDeepSeek({
        ...
        messages: [
          { role: 'system', content: prompt.system },
          {
            ...
            content: attempt === 0
              ? prompt.user
              : `${prompt.user}\nYour previous draft failed review. Follow every rule exactly: the short conditional answer with its lean and one supplied card, correct labels, word bands, at least three supplied cards named with positions across the bodies, no guarantees, no timing.`,
          },
        ],
      ...
      const blocks = Array.isArray(parsed) ? parsed : (Array.isArray(parsed.insights) ? parsed.insights : []);
      const shaped = blocks.map((block, index) => ({
        n: `0${index + 1}`,
        label: clean(block && block.label, 60),
        lead: clean(block && block.lead, 160).replace(/[–—]/g, ','),
        body: clean(block && block.body, 480).replace(/[–—]/g, ','),
      }));
      const answer = clean(parsed && parsed.answer, 400).replace(/[–—]/g, ',');
      const audit = auditInsights(shaped, cards);
      const answerAudit = auditAnswer(answer, cards);
      if (audit.ok && answerAudit.ok) {
        structuredLog('info', {
        ...
        return { answer, insights: shaped, source: attempt === 0 ? 'model_initial' : 'model_retry' };
      }
      structuredLog('info', {
        event: 'sc7_insights_rejected',
        attempt,
        reason: audit.ok ? `answer: ${answerAudit.reason}` : audit.reason,
        readingId: payload.readingId || 'missing',
      });
    } catch (error) {
      structuredLog('warn', {
  ...
  return { answer: fallbackAnswer, insights: fallback, source: 'deterministic_fallback' };
}
...
async function handleFreeReading(request, env) {
  ...
  if (cards) {
    const { answer, insights, source } = await generateInsights({
