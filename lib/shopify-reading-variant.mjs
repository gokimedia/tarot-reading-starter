...
export const SHOPIFY_STOREFRONT_READING_VARIANT_QUOTE_QUERY = `#graphql
  query CheckoutReadingVariantQuote($id: ID!, $country: CountryCode!) @inContext(country: $country) {
    node(id: $id) {
      ... on ProductVariant {
        id
        sku
...
async function verifyStorefrontVariant({
  ...
  try {
    response = await storefrontFetch(`https://${store}/api/${apiVersion}/graphql.json`, {
      ...
      body: JSON.stringify({
        query,
        variables: {
          id: `gid://shopify/ProductVariant/${normalizedId}`,
  ...
  if (!/^[A-Z]{3}$/.test(currency)) return failure('contract', 422, 'SHOPIFY_VARIANT_CURRENCY_INVALID');
  if (currency !== expectedCurrency) return failure('contract', 422, 'SHOPIFY_VARIANT_CURRENCY_MISMATCH');
  if (!localizedQuote && Math.abs(price - normalizedExpectedPrice) > 0.001) {
  ...
  return localizedQuote
    ? { ok: true, quote: { variantId: normalizedId, sku, price, priceCents: Math.round(price * 100), currency, country: countryCode } }
    : { ok: true, product: { variantId: normalizedId, sku, price } };
}
...
export async function verifyShopifyReadingVariantQuote({
  variantId,
  expectedSku,
  countryCode = 'US',
  expectedCurrency = 'USD',
  ...
  const normalizedId = String(variantId || '').trim();
  const normalizedSku = String(expectedSku || '').trim();
  const normalizedCountry = String(countryCode || 'US').trim().toUpperCase();
  const normalizedCurrency = String(expectedCurrency || 'USD').trim().toUpperCase();
  if (!/^\d{8,20}$/.test(normalizedId)
    || !/^READING-(?:DEEP|MEDIUM|PREMIUM)$/.test(normalizedSku)
    || !/^[A-Z]{2}$/.test(normalizedCountry)
    || !/^[A-Z]{3}$/.test(normalizedCurrency)) {
    return failure('contract', 422, 'EXPECTED_VARIANT_QUOTE_CONTRACT_INVALID');
  }
  ...
  const common = {
    ...
    apiVersion: storefrontApiVersion,
    storefrontFetch,
    query: SHOPIFY_STOREFRONT_READING_VARIANT_QUOTE_QUERY,
    countryCode: normalizedCountry,
    expectedCurrency: normalizedCurrency,
    localizedQuote: true,
  };
  let result = await verifyStorefrontVariant({ ...common, token: storefrontToken });
  if (storefrontToken
    && result.ok === false
    && result.reason === 'SHOPIFY_STOREFRONT_HTTP_ERROR'
    && (result.upstreamStatus === 401 || result.upstreamStatus === 403)) {
    result = await verifyStorefrontVariant({ ...common, token: '' });
  }
  // A localized Markets price has no safe Admin fallback: the Admin price is
  // shop-currency catalog data, not the buyer's contextual quote.
  return result;
}
