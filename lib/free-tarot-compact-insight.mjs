...
export function deterministicFreeTarotCompactInsight(contract = {}) {
  ...
  if (locale === 'tr') return `${card}, ${phrase} çevresindeki eski bir örüntünün burada taşıdığın duygusal baskıyı nasıl şekillendirdiğini gösteriyor; ancak daha geniş kişisel tablonun tamamını hâlâ açık bırakıyor.`;
  if (locale === 'de') return `${card} zeigt, wie ein früheres Muster rund um ${phrase} den Druck prägt, den du hier mitbringst, ohne das größere persönliche Bild vollständig festzulegen.`;
  if (locale === 'es') return `${card} muestra cómo un patrón anterior en ${phrase} sigue moldeando la presión que traes aquí, sin definir completamente el panorama personal.`;
  if (locale === 'pt') return `${card} mostra como um padrão anterior em ${phrase} molda a pressão que traz aqui, sem definir todo o panorama pessoal mais amplo.`;
  return `${card} highlights how an earlier pattern around ${phrase} still shapes the emotional pressure you carry here, while leaving the wider picture unresolved.`;
}
