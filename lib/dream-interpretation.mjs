...
export const DREAM_PRIVACY_MODE = 'Temporary processing · raw dream excluded from storage, analytics and checkout';

const FORBIDDEN_DREAM_OUTPUT_TERMS = /\b(?:ai|artificial intelligence|deepseek|chatgpt|language model|model provider|vercel|interpretation model|interpretation service)\b|yapay zek[aâ]/iu;
...
export function safeDreamAiOutput(value) {
  ...
  const visibleText = [
    headline,
    summary,
    ...themes.flatMap((theme) => [theme.reflection, theme.question]),
    ...groundingSteps,
    safetyNote,
  ].join(' ');
  if (FORBIDDEN_DREAM_OUTPUT_TERMS.test(visibleText)) return null;
...
export function dreamEvidence(input, output) {
  ...
  return {
    ...
    context: 'Privacy-minimized result; Deckaura used allowlisted themes, tone and a length band to create this reflection. Raw dream text was not retained or attached to checkout.',
    ...
    confidence: 'Symbolic reflection generated from allowlisted themes; personal meaning may differ.',
  };
}
