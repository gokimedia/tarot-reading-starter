...
export function verifySharedToolPaidOrder(input = {}) {
  ...
  const runeV2Validation = verifyRuneV2PaidLine({
    page,
    toolType: readingType,
    presentationVariant,
    snapshot,
  ...
  if (page === RUNE_V2_PAGE && (!runeV2Validation.applies || !runeV2Validation.ok)) {
    return failure(runeV2Validation.reason || 'RUNE_CHECKOUT_V2_SNAPSHOT_INVALID');
  }
  ...
  const exactQuoteFlow = exactSevenCardPage || exactPersonalDirectPage || directTarotValidation.applies;
  const hasCheckoutQuote = Object.keys(checkoutQuote).length > 0;
  const genericSignedQuoteFlow = hasCheckoutQuote && !exactQuoteFlow;
  if ((exactQuoteFlow || genericSignedQuoteFlow) && (
    ...
    return failure(exactPersonalDirectPage
      ...
      : directTarotValidation.applies
        ...
        : exactSevenCardPage
          ...
          : 'SHARED_CHECKOUT_QUOTE_MISMATCH');
  }
  const localeContext = record(snapshot.localeContext);
  if (genericSignedQuoteFlow && (
    clean(line.variantId, 24) !== clean(checkoutQuote.variantId, 24)
    || clean(line.sku, 80).toUpperCase() !== clean(checkoutQuote.sku, 80).toUpperCase()
    || Number(line.quantity) !== 1
    || line.requiresShipping !== false
    || clean(line.checkoutIntent, 64) !== clean(checkoutQuote.intentId, 64)
    || clean(line.funnelVersion, 128) !== clean(row.funnelVersion, 128)
    || clean(line.selectedPackage, 20) !== clean(row.tier, 20)
    || clean(line.displayedQuoteCents, 20) !== String(Number(checkoutQuote.priceCents))
    || clean(line.displayedQuoteCurrency, 3).toUpperCase() !== clean(checkoutQuote.currency, 3)
    || clean(line.displayedQuoteCountry, 2).toUpperCase() !== clean(checkoutQuote.country, 2)
    || clean(line.signedQuoteCents, 20) !== String(Number(checkoutQuote.priceCents))
    || clean(line.signedQuoteCurrency, 3).toUpperCase() !== clean(checkoutQuote.currency, 3)
    || clean(line.signedQuoteCountry, 2).toUpperCase() !== clean(checkoutQuote.country, 2)
    || clean(line.language, 8).toLowerCase() !== clean(localeContext.language, 8).toLowerCase()
    || clean(line.locale, 24).toLowerCase() !== clean(localeContext.locale, 24).toLowerCase()
    || clean(line.country, 2).toUpperCase() !== clean(localeContext.country, 2).toUpperCase()
    || clean(line.currency, 3).toUpperCase() !== clean(localeContext.currency, 3).toUpperCase()
    || clean(line.market, 64).toLowerCase() !== clean(localeContext.market, 64).toLowerCase()
  )) {
    return failure('SHARED_CHECKOUT_QUOTE_MISMATCH');
  }
  ...
  return {
    ...
    verifiedFields: {
      ...
      curiosityQuestion,
      presentationVariant,
      ...((exactQuoteFlow || genericSignedQuoteFlow) ? {
      ...
      ...(runeV2Validation.applies && runeV2Validation.ok
        ? runeV2Validation.verifiedFields
        : {}),
