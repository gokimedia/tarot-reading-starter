...
export const BIRTH_CARD_DIRECT_FOCUSES = Object.freeze({
...
export function birthCardDirectFocusLabel(value) {
  const focus = cleanText(value, 40);
  return Object.hasOwn(BIRTH_CARD_DIRECT_FOCUSES, focus)
    ? BIRTH_CARD_DIRECT_FOCUSES[focus]
    : '';
}

export function birthCardDirectCanonicalQuestion(questionValue, focusValue = '') {
  const question = cleanText(questionValue, 360);
  if (question) return question;
  const focusLabel = birthCardDirectFocusLabel(focusValue);
  return focusLabel
    ? `What guidance do my birth cards offer for ${focusLabel}?`
    : 'What guidance do my birth cards offer?';
}
...
export function validateDirectTarotToolSnapshot(input = {}) {
  ...
  const curiosityQuestion = cleanText(snapshot.curiosityQuestion, 400);
  const downstreamBirthFocus = config.kind === 'birth' ? cleanText(snapshot.birthCardFocus, 40) : '';
  const displayedFocus = cleanText(snapshot.focus, 160);
  const focus = downstreamBirthFocus || displayedFocus;
  if (curiosityQuestion) return failure('empty_optional_fields_required');
  if (config.kind === 'birth') {
    if (focus && !birthCardDirectFocusLabel(focus)) return failure('birth_card_focus_invalid');
    if (downstreamBirthFocus && displayedFocus !== birthCardDirectFocusLabel(focus)) return failure('birth_card_focus_label_mismatch');
  } else if (focus) {
    return failure('empty_optional_fields_required');
  }
  ...
  if (config.kind === 'yes_no') {
    ...
    const questionSource = cleanText(snapshot.questionSource || snapshot.birthCardQuestionSource, 32);
    const focusLabel = birthCardDirectFocusLabel(focus);
    const suppliedFocusLabel = cleanText(snapshot.focusLabel || snapshot.birthCardFocusLabel, 80);
    if (questionSource && !['customer', 'server_default'].includes(questionSource)) return failure('birth_card_question_source_invalid');
    if (questionSource === 'server_default' && (
      rawQuestion !== birthCardDirectCanonicalQuestion('', focus)
      || suppliedFocusLabel !== focusLabel
    )) return failure('birth_card_default_question_mismatch');
    if (questionSource === 'customer' && !rawQuestion) return failure('birth_card_customer_question_missing');
...
export function canonicalizeDirectTarotSnapshot(input = {}) {
  ...
  const normalized = {
    ...source,
    question: validation.kind === 'birth'
      ? birthCardDirectCanonicalQuestion(source.question, source.focus)
      : cleanText(source.question, validation.kind === 'yes_no' ? 240 : 400),
    context: cleanText(source.context, 500),
    signals: validation.canonicalSignals,
    cards: validation.canonicalSignals,
    curiosityQuestion: '',
    focus: validation.kind === 'birth' ? cleanText(source.focus, 40) : '',
  };
  if (validation.kind === 'yes_no') {
    ...
    const submittedQuestion = cleanText(source.question, 360);
    const submittedQuestionSource = cleanText(source.questionSource, 32);
    normalized.questionSource = !submittedQuestion
      || (submittedQuestionSource === 'server_default'
        && submittedQuestion === birthCardDirectCanonicalQuestion('', normalized.focus))
      ? 'server_default'
      : 'customer';
    normalized.focusLabel = birthCardDirectFocusLabel(normalized.focus);
...
export function auditDirectTarotCompactInsight(value, contract = {}) {
  ...
  if (contract.kind === 'yes_no') {
    const labels = Object.values(ANSWER_LABELS[normalizedLocale(contract.locale)]);
