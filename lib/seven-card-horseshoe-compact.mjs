...
export function sevenCardHorseshoeCheckoutSnapshotFromPreview(preview, now = Date.now()) {
  ...
  return {
    ...
    localeContext: Object.freeze({
      locale: cleanText(fields.locale).slice(0, 24),
      country: cleanText(fields.country).slice(0, 2).toUpperCase(),
      currency: cleanText(fields.currency).slice(0, 3).toUpperCase(),
      market: cleanText(fields.market).slice(0, 64).toLowerCase(),
    }),
  };
}
