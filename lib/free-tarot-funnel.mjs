...
export function deterministicAnswer(cards) {
  ...
  const face = (card) => `${card.name}${card.reversed ? ' reversed' : ''}`;
  if (revCount === 0) {
...
const ANSWER_LEAN = /\b(?:yes|no|lean(?:s|ing)?|points?|suggests?|favou?rs?|likely|unlikely|depends|conditional|open|slowed|not yet|possible)\b/i;
...
export async function generateAnswer(payload, env, options = {}) {
  const fallback = deterministicAnswer(payload.cards);
