...
export function questionLanguage(question) {
  ...
  // TR'ye OZGU harfler (o/u/c Almanca-Portekizce ile paylasiliyor, alma!)
  if (/[ğışĞİŞ]/.test(q) || /\b(?:mi|mı|mu|mü|acaba|neden|nasıl|olacak|dönecek|sever|bitti|kalmalı)\b/i.test(q)) return 'tr';
  if (/[¿¡ñ]/i.test(q) || /\b(?:volver[áa]|conmigo|contigo|ser[áa]|qu[ée]|todav[íi]a|deber[íi]a)\b/i.test(q)) return 'es';
  if (/[äß]/i.test(q) || /\b(?:wird|oder|nicht|wieder|sich|sollte|mich|zur[üu]ck)\b/i.test(q)) return 'de';
  if (/[ãõ]/i.test(q) || /\b(?:vai|comigo|j[áa]|n[ãa]o|voltar|ainda|deveria)\b/i.test(q)) return 'pt';
  return 'en';
}
export function questionIsTurkish(question) { return questionLanguage(question) === 'tr'; }
...
export function deterministicAnswer(cards, question) {
  ...
  const REV = { tr: ' ters', es: ' invertida', de: ' umgekehrt', pt: ' invertida', en: ' reversed' };
  const face = (card) => `${card.name}${card.reversed ? (REV[lang] || REV.en) : ''}`;
  const mid = cards.find((card) => card.reversed) || first;
  const L = {
    tr: [
      `Açılım açık yönde eğilim gösteriyor: ${face(first)} durumun bugünkü zeminini, ${face(last)} ise ileriye çeken gücü anlatıyor; tarif ettiği yön şu an erişilebilir.`,
      `Eğilim koşullu: hareket mümkün, ama ${face(mid)} önce değişmesi gereken tek şeyi işaret ediyor; ${face(last)} ancak ondan sonra yönü taşıyabilir.`,
      `Açılım yavaşlamaya işaret ediyor: ${face(last)} bir duraklama istiyor, ${face(first)} ise karar öncesi mevcut düzenin neden oturması gerektiğini gösteriyor.`,
    ],
    es: [
      `La tirada se inclina abierta: ${face(first)} muestra dónde está la situación hoy y ${face(last)} lleva el impulso hacia adelante; la dirección que describe está disponible ahora.`,
      `La inclinación es condicional: el movimiento es posible, pero ${face(mid)} señala lo único que debe cambiar antes de que ${face(last)} pueda llevar esto adelante.`,
      `La tirada se inclina lenta: ${face(last)} pide una pausa y ${face(first)} muestra por qué el patrón actual necesita asentarse antes de un paso decisivo.`,
    ],
    de: [
      `Die Legung deutet auf offen: ${face(first)} zeigt, wo die Lage heute steht, und ${face(last)} trägt den Zug nach vorn; die beschriebene Richtung ist jetzt möglich.`,
      `Die Tendenz ist bedingt: Bewegung ist möglich, aber ${face(mid)} markiert das eine, das sich zuerst ändern muss, bevor ${face(last)} die Richtung tragen kann.`,
      `Die Legung deutet auf langsam: ${face(last)} bittet um eine Pause, und ${face(first)} zeigt, warum sich das aktuelle Muster erst setzen muss.`,
    ],
    pt: [
      `A tiragem inclina para aberto: ${face(first)} mostra onde a situação está hoje e ${face(last)} carrega o impulso adiante; a direção descrita está disponível agora.`,
      `A inclinação é condicional: o movimento é possível, mas ${face(mid)} aponta a única coisa que precisa mudar antes que ${face(last)} leve isso adiante.`,
      `A tiragem inclina para lento: ${face(last)} pede uma pausa e ${face(first)} mostra por que o padrão atual precisa assentar antes de um passo decisivo.`,
    ],
  };
  if (L[lang]) {
    if (revCount === 0) return L[lang][0];
    if (revCount >= Math.ceil(cards.length / 2)) return L[lang][2];
    return L[lang][1];
  }
  if (revCount === 0) {
  ...
  return `The lean is conditional: movement is possible, but ${face(mid)} marks the one thing that has to shift before ${face(last)} can carry this forward.`;
}

const FORBIDDEN_FREE = /\b(?:will (?:definitely|certainly|surely)|guaranteed?|i promise|(?:he|she|they) (?:loves?|misses?|wants?|is thinking)|garantiert|garantizad[oa]|garantid[oa]|prometo|ich verspreche|kesinlikle dönecek)\b/iu;
...
const ANSWER_LEAN = /\b(?:yes|no|lean(?:s|ing)?|points?|suggests?|favou?rs?|likely|unlikely|depends|conditional|open|slowed|not yet|possible|evet|hayır|henüz|eğilim|bağlı|koşul|açık|yavaş|olası|mümkün|dönebilir|gösteriyor|işaret|sí|inclina|abiert[oa]|lent[oa]|todavía no|posible|apunta|sugiere|favorece|depende|ja|nein|noch nicht|hängt|bedingt|offen|deutet|spricht|möglich|langsam|sim|não|ainda não|possível|aponta|sugere|aberto)\b/iu;
...
function prompt(question, spread, cards, category) {
  ...
  const system = [
    ...
    '- Answer in the language of the question, but keep every card name EXACTLY as supplied in English (for example "Ten of Swords"); never translate card names.',
  ].join('\n');
        ...
        messages: [
          { role: 'system', content: p.system },
          {
            ...
            content: attempt === 0 ? p.user
              : `${p.user}\nYour previous draft failed review. Follow every rule: 16-44 words, bounded lean, at least one supplied card named with its position using its EXACT English card name, answer in the question language, no guarantees, no dates.`,
          },
        ],
        max_tokens: 160,
