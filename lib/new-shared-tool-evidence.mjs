...
const DREAM_CONTEXT = 'Privacy-minimized AI result; raw dream text was processed ephemerally and was not retained by Deckaura or attached to checkout.';
...
const RULES = Object.freeze({
  ...
  '/pages/ai-dream-interpreter': { 'dream themes': /^[A-Za-z][A-Za-z ,'-]{0,180}$/, 'emotional tone': /^(?:curious|anxious|sad|calm|confused)$/, 'dream length band': /^(?:under 50 words|50–149 words|150\+ words)$/, 'privacy mode': /^Local-only free text · raw dream excluded$/ },
...
function validateDream(map, context, snapshot) {
  ...
  if (!exactKeys(map, keys)
    ...
    || map.get('privacy mode') !== 'Ephemeral Vercel AI · raw dream excluded from analytics and paid order') return false;
...
export function validateNewSharedToolSnapshot({ page, toolType, snapshot }) {
  ...
  if (scope !== PAGE_SCOPES[canonicalPage] || confidence !== PAGE_CONFIDENCES[canonicalPage]) {
