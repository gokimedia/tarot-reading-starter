...
const PAGE_TYPES = Object.freeze({
  ...
  '/pages/ai-dream-interpreter': 'Dream Interpretation',
...
const DREAM_CONTEXT = 'Privacy-minimized AI result; Deckaura derived allowlisted themes, tone and a length band on its server, then sent only those signals to DeepSeek. Raw dream text was not sent to the model, retained, or attached to checkout.';
...
const DREAM_CONFIDENCE = 'AI-assisted symbolic reflection grounded in allowlisted themes; personal meaning may differ.';
...
const PAGE_SCOPES = Object.freeze({
  ...
  '/pages/ai-dream-interpreter': DREAM_SCOPE,
...
const PAGE_CONFIDENCES = Object.freeze({
  ...
  '/pages/ai-dream-interpreter': DREAM_CONFIDENCE,
...
const RULES = Object.freeze({
  ...
  '/pages/ai-dream-interpreter': { 'dream themes': /^[A-Za-z][A-Za-z ,'-]{0,180}$/, 'emotional tone': /^(?:curious|anxious|sad|calm|confused)$/, 'dream length band': /^(?:under 50 words|50–149 words|150\+ words)$/, 'privacy mode': /^Server-minimized DeepSeek · raw dream not sent to the model or checkout$/ },
...
function validateDream(map, context, snapshot) {
  ...
  if (!exactKeys(map, keys)
    ...
    || map.get('privacy mode') !== 'Server-minimized DeepSeek · raw dream not sent to the model or checkout') return false;
...
export function validateNewSharedToolSnapshot({ page, toolType, snapshot }) {
  ...
  else if (canonicalPage === '/pages/ai-dream-interpreter') ok = validateDream(map, context, snapshot);
